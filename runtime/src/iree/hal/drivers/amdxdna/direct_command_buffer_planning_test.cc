...
TEST(ApplyPatchTableTest, DoesNotDoubleCountBakedSubBufferOffset) {
  ...
  // a BD addressing a sub-range of a host buffer, the MLIR-AIE compiler bakes
  // the intra-buffer byte offset into the BD address word AND emits the same
  // offset as the DDR_PATCH addend (arg_plus). The patcher must apply the
  // offset once (buffer_base + arg_plus), overwriting the baked value; adding to
  // it would land the descriptor at buffer_base + 2*offset and drop the
  // transfer, zeroing every other worker's output.
  std::vector<uint32_t> ctrl(8, 0);
